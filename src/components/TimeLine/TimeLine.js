...
const Timeline = () => {
  ...
  const handleScroll = () => {
    if (carouselRef.current) {
      const index = Math.round((carouselRef.current.scrollLeft / (carouselRef.current.scrollWidth * 0.7)) * timeline.length);
  ...
  return (
    <Section id="timeline" style={{paddingTop: 45}}>
      <SectionDivider />
      <SectionTitle main>Timeline</SectionTitle>
      <SectionText>A collection of career highlights:</SectionText>
